...
SET HOST_NAME=VECTOR

SET SW_NAME=vector-node
SET SW_PORT=8000
SET SW_IMAGE=cyb3rdog/vector-node:latest
...
REM docker build -t %SW_NAME% .
docker run -it --rm --name %SW_NAME% -h "%HOST_NAME%" -p 80:%SW_PORT% -w /usr/src/app -d %SW_IMAGE%
